...
**MVP核心进度**: 约 **95%** ⬆️ 🎉 (仅缺端到端脚本)  
**完整系统进度**: 约 **80%** ⬆️ (核心检索完成,缺自适应学习)
...
### 🟡 任务组C: 端到端检索脚本 - P1 (唯一剩余)
**状态**: 待实现  
**影响**: 无法一键运行完整检索流程

- [ ] `scripts/08_run_kg_clir.py` - 端到端KG-CLIR检索
...
### 自适应学习模块 - P2 (Future Work)
**状态**: 未实现,可选扩展功能  
**影响**: 不影响论文核心贡献 (CLIR系统)

**待实现文件** (6个,约8-10小时):
- [ ] `adaptive/learner_model/mastery.py` - 掌握度评估
- [ ] `adaptive/learner_model/profile.py` - 学习画像建模
- [ ] `adaptive/path_reco/recommend_path.py` - 学习路径推荐
- [ ] `adaptive/rag_tutor/rag_retrieve.py` - RAG增强辅导
- [ ] `adaptive/rag_tutor/generate_exercise.py` - 智能练习生成
...
- [ ] `adaptive/ablation/run_ablation.py` - 系统化消融实验

**替代方案**: 在`scripts/09_eval_clir.py`中手动调整权重参数:
```python
# 消融实验1: 无KG (γ=0)
reranker = FusionReranker(alpha=0.7, beta=0.3, gamma=0.0)

# 消融实验2: 无Sparse (β=0)
reranker = FusionReranker(alpha=0.7, beta=0.0, gamma=0.3)

# 消融实验3: 无Dense (α=0)
reranker = FusionReranker(alpha=0.0, beta=0.5, gamma=0.5)
```

**建议**: 论文Reviewer可能要求,预留1-2小时实现
...
5. ⏳ `scripts/08_run_kg_clir.py` - 端到端检索 **(唯一剩余)**

**里程碑**: 数据Pipeline完整,仅缺端到端集成脚本
...
- ✅ Phase 2: KG增强 (5/5)
- ✅ Phase 3: 评测系统 (5/5)
- ✅ Phase 4: 数据Pipeline (4/5)

**仅剩1个文件:**
- ⏳ `scripts/08_run_kg_clir.py` - 端到端检索脚本 (1-2小时)

**论文实验状态:**
- ✅ 可立即运行所有实验
- ✅ 可生成论文所需全部数据
- ✅ 可输出LaTeX格式结果表格
- ⏳ 缺少一键演示脚本 (不影响论文,仅影响Demo)
